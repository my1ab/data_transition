...
def get_single_trajectory(env, task_idx, env_idx=0, turns=50, show_turn=False, use_history=True,
                          use_para=0, num_para=1, env_num=50, group_n=1, prompt=1, ds_model=1, effort=1, history_length=50):
    """
    ...
    for turn in range(turns):
        ...
        if prompt == 4:
            ...
            # 使用历史压缩prompt (reason_prompt_para_his)
            # 构建历史信息（压缩摘要）
            if len(step_records) > 0:
                # 历史信息：前面步骤的压缩摘要
                history_info_parts = []
                for i, record in enumerate(step_records[:-1]):
                    history_info_parts.append(f"Step {i+1}: Action: {record['action'][:100]}")
                history_info = "Summary of previous steps:\n" + "\n".join(history_info_parts)
            else:
                history_info = "No previous steps."
            
            # 上一步历史（如果有的话）
            if len(step_records) > 0:
                last_record = step_records[-1]
                last_history = f"Actions: {last_record['action']}\nObservations: {last_record['observation'][:300]}...\nAdmissible Actions: {last_record['admissible_actions'][:200]}..."
            else:
                last_history = "None"
        ...
        if result.get('actions') and len(result['actions']) > 0:
            ...
            # 将所有环境的动作合并为一个字符串记录
            actions_str = ", ".join([f"env_{idx}:{action}" for idx, action in enumerate(normalized_actions)])
            current_obs_str = obs_prompt  # 使用当前构建的obs_prompt
            current_admissible_str = admissible_actions
            step_records.append({
                'step': turn + 1,
                'action': actions_str,
                'observation': current_obs_str,
                'admissible_actions': current_admissible_str
            })
            # 保持step_records长度限制
            if len(step_records) > history_length:
                step_records = step_records[-history_length:]
        else:
...
def generate_coldstart_data(output_file, num_cpus=0.1, end_idx=500, turns=50, use_ray=False, use_para=0, show_turn=False, 
                            ...
                            only_test=0, prompt=1, limit_goals=-1, ds_model=1, effort=1, history_length=50, seed=42,
                            start_index=0):
    """
