...
def get_single_trajectory(env, task_idx,env_idx=0, turns=50,  show_turn=False, use_history=True,
    ...
    for turn in range(turns):
            ...
            if result['action'] and result['action'] != 'None':
                ...
                if normalized_action:
                    ...
                    if len(step_result) == 6:
                        obs_list, reward_list, done_list, info_list, reward, action_valid = step_result
                    elif len(step_result) == 5:
                        obs_list, reward_list, done_list, info_list, action_valid = step_result
                        reward = reward_list[0] if reward_list else None
                    else:
        ...
        if use_para == 0:
            ...
            is_done = done_list[env_idx] if normalized_action else False

            if result['action'] and result['action'] != 'None' and is_done:
                print(f"Task {task_idx} completed at turn {turn + 1} with reward {reward}")
                break
            elif not result['action'] or result['action'] == 'None' or normalized_action is None:
                print(f"Task {task_idx} failed at turn {turn + 1} with reward {reward}")
                break
        else:
            ...
            if any_done:
                ...
                print(f"Task {task_idx} failed at turn {turn + 1}")
                break
...
def generate_coldstart_data(output_file, num_cpus=0.1, num_samples=500, turns=50, use_ray=False, use_para=0, show_turn=False, 
    ...
    for i in tqdm(range(num_samples), desc="Generating coldstart data"):
        try:
            ...
            # 根据最后的 reward 判断轨迹是否成功
            if use_para == 0:
                # 获取轨迹中最后一个 assistant 消息的 reward
                final_reward = 0
                for msg in reversed(trajectory):
                    if msg.get('role') == 'assistant' and 'reward' in msg:
                        final_reward = msg['reward']
                        break
                
                # 判断是否成功（reward > 0 则成功）
                is_success = final_reward > 0 if final_reward is not None else False
                
                # 如果成功，添加到成功轨迹列表
                if is_success:
                    success_data.append(trajectory)
            else:
                # 并行模式：获取轨迹中最后一个 assistant 消息的 rewards
                final_rewards = []
                for msg in reversed(trajectory):
                    if msg.get('role') == 'assistant' and 'rewards' in msg:
                        rewards_data = msg['rewards']
                        # 如果 rewards 是字符串，解析为列表
                        if isinstance(rewards_data, str):
                            final_rewards = json.loads(rewards_data)
                        else:
                            final_rewards = rewards_data
                        break
                
                # 判断是否成功（任一环境的 reward > 0 则成功）
                is_success = False
                if final_rewards:
    ...
    if success_data:
        ...
        with open(log_file, 'w') as f:
            ...
            f.write(f"Mode: {'parallel' if use_para > 0 else 'single'}\n")
            if use_para > 0:
...
if __name__ == "__main__":
    ...
    user = 'xuxz'
    # OUTPUT_FILE = f'/home/dpepo/verl-agent/coldstart_genaration_webshop/WebShop_coldstart.json'
    OUTPUT_FILE = f'/diskpool/home/{user}/verl-agent/coldstart_result_webshop/WebShop_coldstart.json'
    ...
    if move == 1:
        ...
        #                         # use_ray=False, 
        #                         use_para=0,num_para=5, group_n=1,
        ...
        #                         )
        generate_coldstart_data(OUTPUT_FILE, num_samples=50, turns=50, 
                                # use_ray=False, 
                                num_cpus=0.5,
                                use_para=1, num_para=5, group_n=1, env_num=50,
